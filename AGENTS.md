...
- `Chatter/Models/` — SwiftData `@Model` objects: `Agent`, `ChatSession`, `Message`, `MCPServerConfig`, `KnowledgeBundle`, `KnowledgeConcept`, `Skill`, `MemoryEntry`, `ReminderEntry`.
- `Chatter/Services/` — `ChatEngine`, `OllamaService`, `MCPConnectionManager`, `Knowledge/` (OKFCodec, KnowledgeTransfer, KnowledgeToolProvider, PDFKnowledgeImporter, PDFImportJob), `SkillCodec`/`SkillTransfer`/`SkillToolProvider`, `MemoryToolProvider`, `ReminderToolProvider`/`ReminderScheduler` (reminders fire as local notifications; entries with an `actionPrompt` run an agent turn on next app open after the due date), `WebToolProvider`, `KeychainService`, `CloudSyncMonitor`, `AppLogger`, `LegacySSEClientTransport`, plus `Protocols/`.
