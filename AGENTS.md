...
- **Streaming flow** (`ChatEngine.send`):
  ...
  2. Resolve tools: agent's MCP tools via `MCPConnectionManager` (namespaced `server.tool`) plus built-in providers (`knowledge__*`, `memory__*`, `reminders__*`, skill, web search/fetch) owned by `ChatEngine`.
...
- `Chatter/Models/` — SwiftData `@Model` objects: `Agent`, `ChatSession`, `Message`, `MCPServerConfig`, `KnowledgeBundle`, `KnowledgeConcept`, `Skill`, `MemoryEntry`, `ReminderEntry`. (`HandoffRequest` is deliberately NOT a model — it lives outside the mirrored schema, see `SERVER-HANDOFF.md`.)
- `Chatter/Services/` — `ChatEngine`, `OllamaService`, `MCPConnectionManager`, `Knowledge/` (OKFCodec, KnowledgeTransfer, KnowledgeToolProvider, PDFKnowledgeImporter, PDFImportJob), `SkillCodec`/`SkillTransfer`/`SkillToolProvider`, `MemoryToolProvider`, `ReminderToolProvider`/`ReminderScheduler` (reminders fire as local notifications; entries with an `actionPrompt` run an agent turn on next app open after the due date), `WebToolProvider`, `KeychainService`, `CloudSyncMonitor`, `AppLogger`, `LegacySSEClientTransport`, `TurnRuntimeKeeper` (keeps turns alive in the background: iOS 26 `BGContinuedProcessingTask` Live Activity, else `beginBackgroundTask`; posts completion notifications), `HandoffChannel`/`HandoffCoordinator`/`HandoffServer` (turn handoff to a Mac via direct CloudKit reads/writes — not SwiftData mirroring, see `SERVER-HANDOFF.md`), plus `Protocols/`.
- `Chatter/Views/` — `RootView` + `Sidebar/`, `Chat/`, `Agents/`, `Knowledge/`, `Skills/`, `Settings/`, `Components/`. Platform diffs inline via `#if os(macOS)`.
- `ChatterWatch/` — watchOS app (own `@main` + views; session list, chat, dictation composer). Shares `Chatter/Models`, `Chatter/Services` (minus PDF importer + `ImageAttachmentProcessor`), `Persistence.swift`, `AppEnvironment.swift`, `ChatViewModel.swift`, `DesignSystem/Theme.swift` via explicit source paths in `project.yml` — no settings UI: agents/MCP/sessions sync via CloudKit; the API key is pulled from the paired iPhone via WatchConnectivity (`WatchKeySync`) because iCloud Keychain does not reliably sync to watchOS.
